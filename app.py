...
def setup_llm_chain(topic="technology"):
    prompt = ChatPromptTemplate.from_messages([
         {"role": "system", "content": "You are a joking AI. Give me only one funny joke about the given topic."},
         {"role": "user", "content": f"generate a joke on the topic: {topic}"}
    ])
...
async def start(update: Update,context: ContextTypes.DEFAULT_TYPE):
    # delay of 5 sec
    await update.message.reply_text("Hi! Mention me with a topic like '@Jokers_rules_bot trees' to get a funny joke.")

async def help_command(update: Update,context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(" Mention me with a topic like @Jokers_rules_bot trees, to get a funny joke")
